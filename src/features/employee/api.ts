...
export const employeeApi = {
  search: (
    ...
    http.get<PageResponse<Employee>>("/employees", {
      params: { ...params, ...page },
    }),
  detail: (employeeId: string) =>
    http.get<Employee>(`/employees/${employeeId}`),
  create: (payload: EmployeePayload) =>
    http.post<Employee>("/employees", payload),
  update: (employeeId: string, payload: EmployeePayload) =>
    http.put<Employee>(`/employees/${employeeId}`, payload),
